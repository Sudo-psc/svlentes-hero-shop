            ...
            try {
                switch (name) {
                    ...
                    case 'n8n_get_workflow':
                        ...
                        const workflow = await this.n8nClient.getWorkflow(args.workflow_id);
                        return {
                            content: [
                                {
                                    type: 'text',
                    ...
                    case 'n8n_execute_workflow':
                        if (!args)
                            throw new Error('Arguments required');
                        const execution = await this.n8nClient.executeWorkflow(args.workflow_id, args.data);
                        return {
                            content: [
                                {
                                    type: 'text',
                    ...
                    case 'n8n_get_executions':
                        const executions = await this.n8nClient.getExecutions(args?.workflow_id);
                        return {
                            content: [
                                {
                                    type: 'text',
                    ...
                    case 'n8n_get_execution':
                        if (!args)
                            throw new Error('Arguments required');
                        const executionDetail = await this.n8nClient.getExecution(args.execution_id);
                        return {
                            content: [
                                {
                                    type: 'text',
                    ...
                    case 'n8n_activate_workflow':
                        if (!args)
                            throw new Error('Arguments required');
                        const activated = await this.n8nClient.activateWorkflow(args.workflow_id);
                        return {
                            content: [
                                {
                                    type: 'text',
                    ...
                    case 'n8n_deactivate_workflow':
                        if (!args)
                            throw new Error('Arguments required');
                        const deactivated = await this.n8nClient.deactivateWorkflow(args.workflow_id);
                        return {
                            content: [
                                {
                                    type: 'text',
