// WebSocket Manager com backoff exponencial e tratamento robusto de erros
// Para uso no lado do cliente (browser)
...
export class WebSocketManager {
  ...
  // Conectar a um WebSocket com retry automático
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        ws.onopen = (event) => {
          ...
          // Configurar listeners após conexão bem-sucedida
        ...
        ws.onclose = (event) => {
          ...
          // Tentar reconectar se não foi uma fechamento normal
...
// Hook React para usar o WebSocket Manager
export function useWebSocket(url: string, config: WebSocketConfig = {}) {
  ...
  useEffect(() => {
    wsManager.connect(url, {
      ...config,
      onOpen: (event) => {
        setConnection(event.currentTarget as WebSocket)
        setIsConnected(true)
        setError(null)
    ...
    return () => {
      wsManager.disconnect(url, config)
    }
  }, [url])

  const sendMessage = useCallback((data: string | ArrayBufferLike | Blob) => {
    return wsManager.send(url, data, config)
  }, [url, config])
